...
| `<class>` | Active G109 register entries; initial set: `G10.970` through `G10.976` | No new class without a formal G109 entry. |
...
| `<evidence-hash>` | SHA-256 digest prefix, 12 through 64 hex chars | Must bind the latest evidence package. |
...
- the behavior can be mapped to `B0` through `B3` or to instrumented `B4`;
...
- escalation triggers are mapped to G10.972 through G10.976 targets;
